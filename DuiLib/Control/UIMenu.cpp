...
namespace DuiLib
{
	...
	{
		m_pMenuWnd->Create(ownerWnd, _T("Menu"),WS_POPUP,WS_EX_NOACTIVATE | WS_EX_TOPMOST | WS_EX_TOOLWINDOW);
	...
	{
		CLabelUI* pControl = (CLabelUI*)m_pManager->FindSubControlByName(pMenuItem, _T("menu_label"));
		pMenuItem->SetBkColor(0xFF191919);
	...
	{
		CLabelUI* pControl = (CLabelUI*)m_pManager->FindSubControlByName(pMenuItem, _T("menu_label"));
		pMenuItem->SetBkColor(0xFF3E3E3E);
	...
	void CDuiMenu::LoadFromXml(LPCTSTR xmlName)
	{
		...
		m_menus = static_cast<CMenuContainer*>(dlgBuilder.Create(xmlName, NULL, this));
