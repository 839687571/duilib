...
CContainerUI* CListUI::GetHeader() const
{
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("headerbkimage")) == 0) ((CListHeaderUI*)GetHeader())->SetBkImage(pstrValue);
...
// 		pImgNode->SetName(_T("menuRawList"));
	...
	//static_cast<CListContainerElementUI*>(m_pDragingCtrl)->GetItemAt(0)->SetText("请选择位置");
}
...
void CListBodyUI::SetScrollPos(SIZE szPos)
{
    ...
    if( cx != 0 && m_pOwner ) {
		CListHeaderUI* pHeader = (CListHeaderUI *)m_pOwner->GetHeader();
...
void CListContainerElementUI::SetOnlyBk(bool  obkcolor)
{
  m_bOnlyBk = obkcolor;
}
...
void CListContainerElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("onlybkcolor")) == 0 ) SetOnlyBk(_tcscmp(pstrValue, _T("true")) == 0);
...
void CListContainerElementUI::DrawItemDivLien(HDC hDC, const RECT& rcItem)
{
	...
	if (pInfo->dwLineColor == 0)return;

	int c = GetCount();
	for (int i = 0; i < c; i++)
	{
		CControlUI *cont1 = GetItemAt(i);
		int w = cont1->GetWidth();
		int h = cont1->GetHeight();
		RECT ctrRect = cont1->GetItemRect();
// 		RECT rcLine = { ctrRect.left, ctrRect.bottom - 1, ctrRect.right, ctrRect.bottom - 1 };
// 		CRenderEngine::DrawLine(hDC, rcLine, 1, GetAdjustColor(pInfo->dwLineColor));

		RECT rcLineLeft = { ctrRect.right - 1, ctrRect.top, ctrRect.right - 1, ctrRect.bottom };
		CRenderEngine::DrawLine(hDC, rcLineLeft, 1, GetAdjustColor(pInfo->dwLineColor));
		//TRACE();
	}
}
} // namespace DuiLib
