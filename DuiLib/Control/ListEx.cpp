...
int CListUIEx::InsertItem(int nItem, int nHeight,std::string listName)
{
	...
	bool CListUIEx::RemoveItem(int nItem)
	{
		return 	CListUI::RemoveAt(nItem);
	...
	{
		CHorizontalLayoutUI *pSubHor = GetListSubItem(nItem, nColumn);
		CControlUI *sub = pSubHor->GetItemAt(0);
		CDuiString subname = sub->GetName();
		CLabelUI *plbl = (CLabelUI *)pSubHor->FindSubControl(itemName);
		plbl->SetText(Text);
	}
